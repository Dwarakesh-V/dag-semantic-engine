...
def depth_pass(enc_query:tensor,query:str="",level=0):
    ...
    if not any(confidences) > lct:
        # Retrieval augmented fallback context - Top 3 historical intent matching
        confs = []
        for child in node.children:
            confs.append(util.cos_sim(MODEL.encode(PREV_DATA),child.intent).max().item())
        confidences[confs.index(max(confs))] += 0.1*max(confs)
        if not any(confidences) > lct:
            if len(PASS_PREV_DATA)==1:
                print("Your query was unable to be processed. Please try rephrasing it.")
                return
            else:
                PASS_PREV_DATA.pop(-1)
